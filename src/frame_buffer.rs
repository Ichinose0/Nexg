...
impl FrameBuffer {
    pub fn new(device: &Device,descriptor: &FrameBufferDescriptor) -> Self {
        ...
        let create_info = FramebufferCreateInfo::builder().width(descriptor.width).height(descriptor.height).layers(1).render_pass(render_pass.render_pass).attachments(&[image_view.image_view]).build();
